#' gearPlotHH.wgdoSH Wing Spread vs. DoorSpread
#'
...
#' @param esc.mult: adjusts the size of text in graphs
...
#' }
#' @export
gearPlotHH.wgdoSH<-function(Survey,years,quarter,c.int=.9,c.inta=.8,c.intb=.8,int.type=c("prediction","confidence","both"),es=FALSE,col1="darkblue",col2="steelblue2",esc.mult=1,getICES=T,pF=T,ti=T) {
  ...
  if (length(subset(dumb,DoorSpread>c(-9)))>0){
     ...
     if (length(unique(dumb$SweepLngt))<2) {
            ...
            # else
            lm.WingVsDoor<-lm(WingSpread~DoorSpread,dumb,subset=c(HaulVal=="V" & WingSpread>0 & DoorSpread>0))
                        #outlierTest(lm.WingVsDoor,data=dumb)
            ds<-data.frame(DoorSpread=seq(dspr[1],dspr[2],length.out = 10))
            plot(WingSpread~DoorSpread,dumb,type="n",subset=HaulVal=="V",xlim=c(dspr[1]-20,dspr[2]+20),ylim=c(wspr[1]-10,wspr[2]+10),xlab=ifelse(es,"Abertura puertas (m)","Door Spread (m)"),ylab=ifelse(es,"Abertura calones (m)","Wing Spread (m)"),pch=21,col="grey",cex.axis=1*esc.mult,cex.lab=1*esc.mult)
            if (pF) {
              points(WingSpread~DoorSpread,dumb,subset=HaulVal=="V")
              legend("bottomright",as.character(paste(years[1],"-",years[length(years)])),pch=21,col=col1,bty="n",inset=.04,cex=1*esc.mult)
              }
            if (ti) title(main=paste0(ifelse(es,"Abertura de calones vs. Abertura de puertas","Wing Spread vs. door spread in "),dumb$Survey[1],".Q",quarter),line=2.5,cex=1.1*esc.mult)
            mtext(dumb$Ship[1],line=.4,cex=.8*esc.mult,adj=0)
            if (pF) {points(WingSpread~DoorSpread,dumb,subset=c(HaulVal=="V"),pch=21)}
            ds<-data.frame(DoorSpread=seq(dspr[1],dspr[2],length.out = 10))
         ...
         if (length(levels(factor(dumb$SweepLngt)))==2) {
           ...
           # }
            ...
            mtext(dumb$Ship[1],line=.4,cex=.8*esc.mult,adj=0)
            if (pF){
              points(WingSpread~DoorSpread,dumb,subset=c(HaulVal=="V" & SweepLngt==levels(factor(SweepLngt))[1]),pch=21,col=col2,bg=col2)
              points(WingSpread~DoorSpread,dumb,subset=c(HaulVal=="V" & SweepLngt==levels(factor(SweepLngt))[2]),pch=21,col=col1,bg=col1)
              ...
              # if (length(years)>1) legend("bottomright",c(paste(c(paste(years[1],years[length(years)-1],sep="-"),years[length(years)]),c(ifelse(es,"Malletas cortas","Short sweeps")),sep=" "),
              #                                             paste(c(paste(years[1],years[length(years)-1],sep="-"),years[length(years)]),
              #                                                                                                                                                                                        c(ifelse(es,"Malletas largas","Long sweeps")),sep=" ")),pch=21,col=c(col2,col1,col1,col1),pt.bg=c(NA,col2,NA,col1),bty="n",inset=c(.02),ncol=2)
              # else {
            ...
            legend("bottomleft",legend=substitute(paste(WSshort == a + b %*% DSshort),list(a=round(coef(lm.WingVsDoor.short)[1],2),b=(round(coef(lm.WingVsDoor.short)[2],2)))),inset=c(.09,.1),bty="n",text.font=2,text.col=col1,cex=1*esc.mult)
