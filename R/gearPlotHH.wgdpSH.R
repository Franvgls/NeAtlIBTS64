...
#' }
#' @export
gearPlotHH.wgdpSH<-function(Survey,years,quarter,c.inta=.8,c.intb=.3,int.type=c("prediction","confidence","both"),es=FALSE,col1="darkblue",col2="steelblue2",esc.mult=1,getICES=TRUE,pF=TRUE,ti=TRUE) {
  ...
  dumb$sweeplngt[dumb$SweepLngt>0]<-factor(dumb$SweepLngt[dumb$SweepLngt>0])
      ...
      if (length(subset(dumb$WingSpread,dumb$WingSpread> c(-9)))>0){
         ...
         if (length(years)>1) {plot(WingSpread~Depth,dumb,xlim=c(0,dpthA[2]+20),ylim=c(0,wspr[2]+10),type="n",subset=WingSpread!=c(-9) & Year!=years[length(years)],pch=21,col=grey(.5),ylab=ifelse(es,"Abertura calones (m)","Wing spread (m)"),xlab=ifelse(es,"Profundidad (m)","Depth (m)"),cex.lab=1*esc.mult,cex.axis=1*esc.mult)
           if(pF) {
             points(WingSpread~Depth,dumb,subset=WingSpread!=c(-9),pch=21,col=col1)
              }
           }
         if (length(years)==1) {plot(WingSpread~Depth,dumb,xlim=c(0,dpthA[2]+20),ylim=c(0,wspr[2]+10),type="n",subset=WingSpread!=c(-9) & WingSpread>0,pch=21,col=grey(.5),ylab=ifelse(es,"Abertura calones (m)","Wing spread (m)"),xlab=ifelse(es,"Profundidad (m)","Depth (m)"),cex.lab=1*esc.mult,cex.axis=1*esc.mult)
           if (pF) {
         ...
         if (length(levels(factor(dumb$sweeplngt)))<2) {
            ...
            if (length(years)>1) WingSpread.log<-nls(WingSpread~a1+b1*log(Depth),dumb,start=c(a1=.1,b1=1),subset=WingSpread!=c(-9) & WingSpread>0)
         ...
         if (length(levels(factor(dumb$sweeplngt)))==2) {
            ...
            if (ti) title(paste0(ifelse(es,"Abertura calones vs. profundidad en ","Wing Spread vs. Depth in "),dumb$Survey[1],".Q",quarter),line=2.5,cex=1.1*esc.mult)
            mtext(dumb$Ship[1],line=.4,cex=.8*esc.mult,adj=0)
            ...
            if (pF) {
              ...
              legend("bottomleft",legend=substitute(WSshort == a1st + b1st %*% log(depth),list(a1st=round(coef(WingSpreadst.log)[1],2),b1st=(round(coef(WingSpreadst.log)[2],2)))),bty="n",text.font=2,inset=c(.05,.1),cex=1*esc.mult)
