...
#' @param esc.mult: changes the size of texts in graph
...
#' }
#' @export
gearPlotHH.nodpSH<-function(Survey,years,quarter,c.inta=.8,c.intb=.3,int.type=c("prediction","confidence","both"),es=FALSE,col1="darkblue",col2="steelblue2",esc.mult=1,getICES=TRUE,pF=TRUE,ti=TRUE) {
      ...
      plot(Netopening~Depth,dumb,xlim=c(0,dpthA[2]+20),ylim=c(0,vrt[2]+2),type="n",pch=21,col=col1,ylab=ifelse(es,"Abertura vertical","Vertical opening (m)"),
         xlab=ifelse(es,"Profundidad (m)","Depth (m)"),subset= Netopening> c(-9),cex.lab=1*esc.mult,cex.axis=1*esc.mult)
          ...
          if (length(levels(dumb$sweeplngt))<2) {
            ...
            if (ti) title(main=paste0(ifelse(es,"Abertura vertical vs. profundidad en ","Vertical opening vs. Depth in "),
                                     dumb$Survey[1],".Q",ifelse(length(quarter)>1,paste0(quarter,collapse="&"),quarter)," survey"),line=2.5,cex.main=1.1*esc.mult)
            mtext(dumb$Ship[length(dumb$Ship)],line=.4,cex=.9*esc.mult,adj=0)
...
#           }
           ...
           if (ti) title(main=paste(ifelse(es,"Abertura vertical vs. profundidad en ","Vertical opening vs. Depth in "),dumb$Survey[1],".Q",
                                     ifelse(length(quarter)>1,paste(quarter,collapse=" "),quarter)),line=2.5,cex.main=1.1*esc.mult)
           mtext(dumb$Ship[1],line=.4,cex=.9*esc.mult,adj=0)
           ...
           legend("topleft",legend=substitute(SortVop == a1st + b1st %*% log(depth),list(a1st=round(coef(Netopeningst.log)[1],2),b1st=(round(coef(Netopeningst.log)[2],2)))),bty="n",text.font=2,inset=.05,cex=1*esc.mult)
